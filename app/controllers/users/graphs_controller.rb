class Users::GraphsController < ApplicationController
	...
	def index
		@search = current_user.events.search(params[:q])
      	...
      	@genres_graph_list = Genre.where(user_id: current_user.id).page(params[:page]).per(5)
	      ...
	      event_months = Event.where(genre_id: params[:genre_id]).where(start_date: DateTime.now.beginning_of_month..DateTime.now.end_of_month).group(:title).sum(:value)
	      @event_now_month = []
	      event_months.sort.each do |event_month|
	      ...
	      event_months = Event.where(genre_id: params[:genre_id]).where(start_date: DateTime.now.ago(1.month).beginning_of_month..DateTime.now.ago(1.month).end_of_month).group(:title).sum(:value)
	      @event_2month_ago = []
	      event_months.sort.each do |event_month|
	        array = [event_month[0], event_month[1]]
	      ...
	      event_months = Event.where(genre_id: params[:genre_id]).where(start_date: DateTime.now.ago(2.month).beginning_of_month..DateTime.now.ago(2.month).end_of_month).group(:title).sum(:value)
	      @event_3month_ago = []
	      event_months.sort.each do |event_month|
	        array = [event_month[0], event_month[1]]
